        ...
        <xsl:template match="/">
            <cprovl:policyResponse xmlns:prov="http://www.w3.org/ns/prov#"
                ...
                <xsl:for-each select="Response/Result">
                    <cprovl:Entity id = 'confidenshare:resultId01'>
                ...
                <xsl:for-each select="Response/Result/Attributes/Attribute">
                    <xsl:if test="@AttributeId='urn:oasis:names:tc:xacml:3.0:subject-id'">
                        
                        <cprovl:Agent prov:id ="ex:001">
                        <xsl:attribute name="prov:id">
                            <xsl:value-of select="AttributeValue"></xsl:value-of> 
                        </xsl:attribute>
                    </cprovl:Agent>
                    </xsl:if>
                    
                    <xsl:if test="@AttributeId='urn:oasis:names:tc:xacml:3.0:action-id'">
                        
                        <cprovl:cProcess prov:id ="ex:001">
                            <xsl:attribute name="prov:id">
                                <xsl:value-of select="AttributeValue"></xsl:value-of> 
                            </xsl:attribute>
                        </cprovl:cProcess>
                    ...
                    <xsl:if test="@AttributeId='urn:oasis:names:tc:xacml:3.0:resource-id'">
                        
                        <cprovl:cResource prov:id ="ex:001">
                            <xsl:attribute name="prov:id">
                                <xsl:value-of select="AttributeValue"></xsl:value-of> 
                            </xsl:attribute>
                        </cprovl:cResource>
