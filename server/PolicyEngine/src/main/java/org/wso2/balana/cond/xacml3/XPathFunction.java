...
/**
 * The class that implement all XPath based functions. An XPath expression
 * evaluates to a node-set, which is a set of XML nodes that match the
 * expression. All comparison or other operations on node-sets are performed in
...
public class XPathFunction extends FunctionBase {
	 ...
	 */
	public static final String NAME_XPATH_NODE_COUNT = FUNCTION_NS_3
			+ "xpath-node-count";

	/**
	 * Standard identifier for the xpath-node-match function.
	 */
	public static final String NAME_XPATH_NODE_MATCH = FUNCTION_NS_3
			+ "xpath-node-match";

	/**
	 * Standard identifier for the xpath-node-equal function.
	 */
	public static final String NAME_XPATH_NODE_EQUAL = FUNCTION_NS_3
			+ "xpath-node-equal";
	...
	public XPathFunction(String functionName) {
		super(functionName, getId(functionName), XPathAttribute.identifier,
				false, getNumArgs(functionName), getReturnType(functionName),
				false);
	}
	...
	public EvaluationResult evaluate(List<Evaluatable> inputs,
		...
		case ID_XPATH_NODE_COUNT: {
			...
			Set<Attributes> attributesSet = ((XACML3EvaluationCtx) context)
			...
			if (contextNode != null) {
				...
				try {
					...
					return new EvaluationResult(new IntegerAttribute(
							nodeList.getLength()));
	...
	private NodeList getXPathResults(Node contextNode, String xpathValue)
		...
		return (NodeList) expression.evaluate(contextNode,
