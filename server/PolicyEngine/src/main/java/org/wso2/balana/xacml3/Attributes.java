...
/**
 * Represents the AttributesType XML type found in the context schema. TODO But
...
public class Attributes {
	...
	public static Attributes getInstance(Node root) throws ParsingException {
		...
		if (!DOMHelper.getLocalName(root).equals(
				XACMLConstants.ATTRIBUTES_ELEMENT)) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (DOMHelper.getLocalName(node).equals(
					XACMLConstants.ATTRIBUTES_CONTENT)) {
			...
			} else if (DOMHelper.getLocalName(node).equals(
	...
	// */
	// public void encodeIncludeAttribute(OutputStream output, Indenter
	// indenter) {
	//
	// String indent = indenter.makeString();
	// PrintStream out = new PrintStream(output);
	...
	// * @param indenter an object that creates indentation strings
	// */
	// public void encodeWithIncludedAttributes(OutputStream output, Indenter
	// indenter) {
