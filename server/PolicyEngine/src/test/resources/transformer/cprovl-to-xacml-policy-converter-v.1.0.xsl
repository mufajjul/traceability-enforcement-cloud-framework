...
<xsl:stylesheet xmlns="urn:oasis:names:tc:xacml:3.0:core:schema:wd-17"
							...
							<xsl:for-each select="cprovl:IF/cprovl:target">
										...
										<AllOf>
											<Match
												...
												<AttributeValue
													...
													AttributeId="urn:com:provenance:cloudprovenance:cprovl:1.0:attribute-id:cprov:newvar"
										...
										<AllOf>
											<Match
												...
												<AttributeValue
													...
													DataType="urn:oasis:names:tc:xacml:3.0:data-type:xpathExpression"
													AttributeId="urn:com:provenance:cloudprovenance:cprovl:1.0:attribute-id:cprov:ref">

													<xsl:choose>
														<xsl:when test=" $varType='cprovd:d-ref'">

															<xsl:attribute name="AttributeId">
																<xsl:text>urn:com:provenance:cloudprovenance:cprovl:1.0:attribute-id:cprov:d-ref</xsl:text>
															</xsl:attribute>

														</xsl:when>
														<xsl:otherwise>
															<xsl:attribute name="AttributeId">
																<xsl:text>urn:com:provenance:cloudprovenance:cprovl:1.0:attribute-id:cprov:s-ref</xsl:text>
															</xsl:attribute>
														</xsl:otherwise>
													</xsl:choose>
								...
								<xsl:if test="not ($varType)">
									<AnyOf>
										<AllOf>
											<Match
												MatchId="labs:orange:com:function:ext:xpath-provenance-id-match">
												<AttributeValue
													XPathCategory="urn:com:provenance:cloudprovenance:cprovl:1.0:attribute-category:cprov"
													DataType="urn:oasis:names:tc:xacml:3.0:data-type:xpathExpression">
					...
					<xsl:if test="count(cprovl:SUCH-THAT)!=0">
						<Condition>
							<xsl:for-each select="cprovl:SUCH-THAT/cprovl:statements">
								...
								<xsl:choose>
									<xsl:when test="@cprovl:negate='true'">
										<Apply FunctionId="urn:oasis:names:tc:xacml:1.0:function:not">
											<Apply FunctionId="urn:oasis:names:tc:xacml:1.0:function:and">
												<xsl:for-each select="*">
													<xsl:variable name="nodeName" select="current()/name()" />
													<xsl:if test="$nodeName = 'cprovl:grouping'">
														<xsl:call-template name="grouping" />
													</xsl:if>
													<xsl:if test="$nodeName = 'cprovl:operator'">
														<xsl:call-template name="operator" />
													</xsl:if>
													<xsl:if test="$nodeName = 'cprovl:statement'">
	...
	<xsl:template name="statement" match="cprovl:statement"
		mode="loop">
		...
		<xsl:if test="@cprovl:negate='true'">
			<Apply FunctionId="urn:oasis:names:tc:xacml:1.0:function:not">
				<Apply FunctionId="labs:orange:com:function:ext:xpath-provenance-node-match">
					<AttributeValue
						DataType="urn:oasis:names:tc:xacml:3.0:data-type:xpathExpression"
						AttributeId="urn:com:provenance:cloudprovenance:cprovl:1.0:attribute-id:cprov"
	...
	<xsl:template name="operator" match="cprovl:operator"
		mode="loop">
	...
	<xsl:template name="grouping" match="cprovl:grouping"
		mode="loop">
